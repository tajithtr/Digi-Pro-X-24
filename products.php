...
$sql = "SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.is_disabled = 0";
$params = [];

if (!$user_logged_in || !$is_sri_lanka) {
    $sql .= " AND p.product_type != 'local_service'";
}

if ($selected_category == 0) {
    // Hide services from the generic "All Products" view
    $sql .= " AND p.product_type != 'local_service'";
}
...
<head>
    ...
    <title>Buy <?php echo htmlspecialchars($current_cat_name); ?> Online in Sri Lanka | DigiPro X24</title>
    <meta name="description" content="Explore <?php echo htmlspecialchars($current_cat_name); ?> at DigiPro X24 Sri Lanka. High quality laptops, PC hardware, accessories & electronics with islandwide cash on delivery.">
    <link rel="canonical" href="https://digiprox24.com/products.php<?php echo $selected_category > 0 ? '?category=' . (int)$selected_category : ''; ?>">
    
    <!-- Open Graph / Facebook -->
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://digiprox24.com/products.php<?php echo $selected_category > 0 ? '?category=' . (int)$selected_category : ''; ?>">
    <meta property="og:title" content="Buy <?php echo htmlspecialchars($current_cat_name); ?> Online in Sri Lanka | DigiPro X24">
    <meta property="og:description" content="Shop <?php echo htmlspecialchars($current_cat_name); ?> at DigiPro X24. Best prices, warranty, and fast islandwide delivery in Sri Lanka.">
    <meta property="og:image" content="https://digiprox24.com/logo.png">
    <meta property="og:site_name" content="DigiPro X24">

    <!-- Twitter -->
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="https://digiprox24.com/products.php<?php echo $selected_category > 0 ? '?category=' . (int)$selected_category : ''; ?>">
    <meta property="twitter:title" content="Buy <?php echo htmlspecialchars($current_cat_name); ?> Online in Sri Lanka | DigiPro X24">
    <meta property="twitter:description" content="Shop <?php echo htmlspecialchars($current_cat_name); ?> at DigiPro X24. Best prices, warranty, and fast islandwide delivery in Sri Lanka.">
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "CollectionPage",
      "name": "<?php echo htmlspecialchars($current_cat_name); ?>",
      "url": "https://digiprox24.com/products.php<?php echo $selected_category > 0 ? '?category=' . (int)$selected_category : ''; ?>",
      "description": "Browse <?php echo htmlspecialchars($current_cat_name); ?> on DigiPro X24 Sri Lanka."
    }
    </script>
    <script type="application/ld+json">
    {
      ...
      "itemListElement": [
        ...
        <?php if ($selected_category > 0): ?>,
        {
          ...
          "position": 3,
          "name": "<?php echo htmlspecialchars($current_cat_name); ?>",
          "item": "https://digiprox24.com/products.php?category=<?php echo (int)$selected_category; ?>"
        }
        <?php endif; ?>
      ]
    }
...
<body>
    ...
    <main class="products-page">
        <div class="products-header-row">
            <h1><span><?php echo htmlspecialchars($current_cat_name); ?></span> Catalog</h1>
        ...
        <div class="shop-container">
            <aside class="sidebar glass-panel">
                ...
                <ul class="category-list">
                    <li>
                        <a href="products.php" class="<?php echo $selected_category == 0 ? 'active' : ''; ?>">
                            All Products
                        </a>
                    </li>
                    <?php foreach($categories as $category): ?>
                        <li>
                            <a href="products.php?category=<?php echo $category['id']; ?>" class="<?php echo $selected_category == $category['id'] ? 'active' : ''; ?>">
                ...
                <div class="mobile-category-wrapper">
                    <select class="mobile-category-select" onchange="location = this.value;">
                        <option value="products.php" <?php echo $selected_category == 0 ? 'selected' : ''; ?>>All Products</option>
                        <?php foreach($categories as $category): ?>
                            <option value="products.php?category=<?php echo $category['id']; ?>" <?php echo $selected_category == $category['id'] ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($category['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <section class="products-content">
                <?php if(count($products) === 0): ?>
                    <div class="glass-panel" style="padding: 4rem; text-align: center; border-radius: 20px;">
                        ...
                        <p style="color: var(--text-muted);">We couldn't find any products matching your selection. Try checking other categories or query terms.</p>
                        <a href="products.php" class="btn-primary" style="display: inline-block; margin-top: 1.5rem; padding: 0.8rem 2rem;">View All Products</a>
                    </div>
                <?php else: ?>
                    <div class="product-grid">
                        <?php foreach($products as $product): 
                            ...
                            $is_flash_sale = (!empty($product['flash_sale_start']) && !empty($product['flash_sale_end']) && strtotime($product['flash_sale_start']) <= time() && strtotime($product['flash_sale_end']) > time());
                            
                            $discount_percent = $product['discount_percent'] ?? 0;
                            $current_price_lkr = $original_price_lkr;
                            
                            if ($is_flash_sale && !empty($product['flash_sale_price'])) {
        ...
        <div class="modal-card" onclick="event.stopPropagation()">
            ...
            <div class="modal-image" style="display: flex; flex-direction: column; position: relative;">
                <div style="position: relative; width: 100%; height: 300px; border-radius: 16px; overflow: hidden;">
                    <img id="qv-image" src="" alt="Product Quick View" style="width: 100%; height: 100%; object-fit: cover;">
