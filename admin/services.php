...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_action']) && isset($_POST['product_ids'])) {
    ...
    try {
        if ($action === 'delete') {
            ...
            $msg = [];
            if ($deleted_count > 0) $msg[] = "$deleted_count service(s) deleted successfully.";
            if ($disabled_count > 0) $msg[] = "$disabled_count service(s) have request/order history, so they were disabled & archived instead.";
            $success = implode(" ", $msg);
        } elseif ($action === 'disable') {
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $stmt = $pdo->prepare("UPDATE products SET is_disabled = 1 WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $success = count($ids) . " services disabled.";
        } elseif ($action === 'enable') {
            $placeholders = implode(',', array_fill(0, count($ids), '?'));
            $stmt = $pdo->prepare("UPDATE products SET is_disabled = 0 WHERE id IN ($placeholders)");
            $stmt->execute($ids);
            $success = count($ids) . " services enabled.";
        }
    } catch (PDOException $e) {
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if ($has_orders) {
            ...
            $success = "This service has request/order history so it cannot be permanently deleted. It has been disabled & archived instead to preserve records.";
        } else {
            $stmt = $pdo->prepare("DELETE FROM products WHERE id = ?");
            $stmt->execute([$delete_id]);
...
<head>
    ...
    <title>Manage Services - Digi Pro X 24 Admin</title>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <div style="display: flex; align-items: center; gap: 1rem; flex-wrap: wrap;">
                <a href="service_add.php" class="btn-add">➕ Add Service</a>
        ...
        <form action="services.php?page=<?php echo $page; ?>" method="POST" id="bulk-actions-form">
                    ...
                    <table class="admin-table responsive-cards">
                        ...
                        <tbody>
                            <?php if (empty($products)): ?>
                                <tr>
                                    <td colspan="7" style="text-align: center; color: var(--text-muted); padding: 2rem;">No services found in the database. Add your first service above!</td>
                                </tr>
                            <?php else: ?>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="td-actions action-links" style="text-align: right; display: flex; gap: 0.5rem; justify-content: flex-end; white-space: nowrap;">
                                            <a href="service_edit.php?id=<?php echo $p['id']; ?>" class="btn-small btn-edit">✏️ Edit</a>
                                            <a href="services.php?delete=<?php echo $p['id']; ?>" class="btn-small btn-delete" onclick="return confirm('Are you sure you want to delete this service? All variants associated with it will also be deleted.');">🗑️ Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
