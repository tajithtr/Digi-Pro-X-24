...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if ($name !== '') {
        ...
        $stmt->execute([$name, $category_image, $type]);
...
if (isset($_GET['delete'])) {
    ...
    $prodCount = $checkStmt->fetchColumn();
    
    if ($prodCount > 0) {
        $error = "Cannot delete category: It contains {$prodCount} product(s). Remove them first.";
    } else {
        ...
        if ($catImg && strpos($catImg, 'uploads/') !== false) {
            $oldPath = '../' . $catImg;
...
<head>
    ...
    <title>Manage Categories - Digi Pro X 24 Admin</title>
            ...
            <div class="panel-box">
                ...
                <form action="categories.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-input" placeholder="e.g. Charging Cables" required>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label" for="image">Category Photo</label>
                    ...
                    <table class="admin-table responsive-cards">
                        ...
                        <tbody>
                            <?php if (empty($categories)): ?>
                                <tr>
                                    <td colspan="5" style="text-align: center; color: var(--text-muted);">No categories found. Add one on the left.</td>
                                </tr>
                            <?php else: ?>
                                ...
                                ?>
                                    <tr>
                                        <td class="td-image">
                                            <img src="<?php echo htmlspecialchars($imgSrc); ?>" alt="<?php echo htmlspecialchars($cat['name']); ?>" class="cat-thumb">
                                        </td>
                                        <td class="td-info" style="font-weight: 600; color: var(--text-main);"><?php echo htmlspecialchars($cat['name']); ?></td>
                                        <td>
                                        ...
                                        <td class="td-actions action-links" style="text-align: right; display: flex; gap: 0.5rem; justify-content: flex-end;">
